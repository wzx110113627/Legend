...
function CCBuilderReaderLoad(strFilePath,proxy,owner)
    ...
    if nil ~= owner then
        ...
        for i = 1, ownerOutletNames:count() do
            ...
            -- LOOTNODE BEGIN --
            if not owner.__ignore_Parse_LootNode__ and "__L_lootNode__" == outletName:getCString() then
                lastLootNode = outletNode
                
                -- print("I WANT YOU!")
            elseif lastLootNode and (
                "L_spBg" == outletName:getCString()
                or "L_spIcon" == outletName:getCString()
                or "L_spIcon" == outletName:getCString()
                or "L_spQuality" == outletName:getCString()
                or "L_labNum" == outletName:getCString()
                or "L_labName" == outletName:getCString()) then
                    
                    lastLootNode[outletName:getCString()] = outletNode
                    
            -- LOOTNODE END --
            else
                if lastLootNode then
                    LootNode.extend(lastLootNode, lastLootNode)
                    lastLootNode = nil
                end
    ...
    local nodesWithAnimationManagers = tolua.cast(ccbReader:getNodesWithAnimationManagers(),"CCArray") or CCArray:create()
    local animationManagersForNodes  = tolua.cast(ccbReader:getAnimationManagersForNodes(),"CCArray") or CCArray:create()
